...
<html lang="en">
  ...
  <script>location.replace(${JSON.stringify(target)});</script>
        ...
        console.error("Error redirecting /signup:", err?.stack || err);
...
async function reconcilePendingPayments() {
    try {
        if (!razorpayClient)
            return;
        const minutes = parseInt(process.env.PAYMENT_RECONCILE_MINUTES || "15", 10) || 15;
        ...
        if (!pendingOrders || pendingOrders.length === 0)
            return;
        console.log(`Pending reconciliation: found ${pendingOrders.length} order(s) older than ${minutes} minute(s)`);
            ...
            try {
                ...
                if (capturedPayment) {
                    ...
                    try {
                        ...
                        if (fullOrder)
                            await sendAllNotifications(fullOrder);
                    }
                    catch (err) {
                        console.error("Failed to send notifications for reconciled order:", err);
                    }
                }
                else {
