...
router.post("/add", authenticateUser, async (req, res) => {
    try {
        ...
        const signedImageUrls = await getMultipleImageUrls(cartItem.product.images || []);
        res.status(200).json({
...
router.get("/", authenticateUser, async (req, res) => {
    try {
        ...
        // Calculate totals
        const totalItems = cartItemsWithSignedUrls.reduce((sum, item) => sum + item.quantity, 0);
        const totalAmount = cartItemsWithSignedUrls.reduce((sum, item) => sum + item.product.price * item.quantity, 0);
        res.json({
