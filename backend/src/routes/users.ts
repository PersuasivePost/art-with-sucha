...
router.put("/me", authenticateUser, async (req: AuthRequest, res) => {
  try {
    ...
    const { name, phone, address, mobno } = req.body as any;
    const updateData: any = {};
    if (typeof name === "string") updateData.name = name;
    if (typeof phone === "string") updateData.mobno = phone;
    if (typeof mobno === "string") updateData.mobno = mobno;
    ...
    const { password: _p, ...publicUser } = updated as any;
    res.json(publicUser);
  } catch (err) {
