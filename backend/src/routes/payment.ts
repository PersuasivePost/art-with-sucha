...
router.post("/verify", authenticateUser, async (req: AuthRequest, res) => {
  try {
    const userId = req.user?.userId;
    ...
    try {
      ...
      if (payment.status !== "captured") {
        console.log(
          `🚫 PAYMENT REJECTED - Status: ${payment.status} (not captured)`
        );
        await prisma.order.update({
          where: { id: order.id },
          data: {
      ...
      console.log(
        "✅ PAYMENT VERIFIED AND CAPTURED - Proceeding to mark as paid"
      );
    } catch (fetchError) {
    ...
    try {
      ...
      if (fullOrder) {
        ...
        await sendAllNotifications(fullOrder).catch((e) =>
...
router.post("/failure", authenticateUser, async (req: AuthRequest, res) => {
  try {
    const userId = req.user?.userId;
