...
// Minimal typed shape (lightweight)
type OrderLike = any;
...
function renderCustomerTemplate(order: OrderLike) {
  ...
  if (!t) {
    ...
    const itemsHtml = (order.orderItems || [])
      .map(
        (i: any) =>
          `<li>${i.product?.title || i.productId} × ${i.quantity} - ₹${
            i.price
          }</li>`
      )
      .join("");
    return (
      `<!doctype html><html><body style="font-family: Arial; max-width:600px;margin:0 auto;">` +
      ...
      `<p>Questions? Reply to this email or call us at ${
...
function renderAdminTemplate(order: OrderLike) {
  ...
  return t
    ...
    .replace(
      ...
      (order.orderItems || [])
        .map(
          (i: any) =>
            `<li>${i.product?.title || i.productId} × ${i.quantity}</li>`
        )
        .join("\n")
    );
}

export async function sendEmailToCustomer(order: OrderLike) {
  const html = renderCustomerTemplate(order);
  const from =
    process.env.EMAIL_FROM || process.env.EMAIL || "no-reply@example.com";
...
export async function sendEmailToAdmin(order: OrderLike) {
...
export async function sendTelegramToAdmin(order: OrderLike) {
  const token = process.env.TELEGRAM_BOT_TOKEN;
  const chatId = process.env.TELEGRAM_CHAT_ID;
  if (!token || !chatId) throw new Error("Telegram not configured");

  const items = (order.orderItems || [])
    .map((i: any) => `• ${i.product?.title || i.productId} x${i.quantity}`)
    .join("\n");
...
export async function sendAllNotifications(order: OrderLike) {
  const result: any = {
    customerEmail: false,
    adminEmail: false,
    telegram: false,
